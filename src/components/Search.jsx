...
function Search({
  ...
  useEffect(() => {
    const filters = players.filter((player) =>
    ...
    <div className={styles.search}>
      <div className="nes-field">
        <input
          type="text"
          disabled={win}
          placeholder={`Intento ${playersHistory.length} / 6`}
          className={`nes-input ${win && 'block'}`}
          value={playerName}
        ...
        <div className={styles.results}>
          {playersResult.map((player) => (
            <div onClick={() => handleClick(player)} key={player.id}>
              <PlayerResult name={player.name} avatar={player.photo} />
            </div>
          ))}
        </div>
      )}
